...
export class Noise implements INoiseConnection {
  ...
  constructor(
    ...
    earlyData?: bytes,
    crypto: ICryptoInterface = stablelib,
    ...
    if (staticNoiseKey) {
      ...
      this.staticKeys =
        this.crypto.generateX25519KeyPairFromSeed(staticNoiseKey);
    } else {
   ...
   */
  public async secureOutbound(
    localPeer: PeerId,
    connection: Duplex<Uint8Array>,
    remotePeer: PeerId
    ...
    const conn = await this.createSecureConnection(
      wrappedConnection,
      handshake
    );
  ...
  private async performHandshake(params: HandshakeParams): Promise<IHandshake> {
    ...
    if (tryIK) {
      ...
      try {
        ...
        return await this.performXXFallbackHandshake(
          params,
          payload,
          err.initialMsg as Uint8Array,
          ephemeralKeys
        );
      }
    } else {
  ...
  private async performXXFallbackHandshake(
    ...
    const handshake = new XXFallbackHandshake(
      isInitiator,
      payload,
      this.prologue,
      this.crypto,
